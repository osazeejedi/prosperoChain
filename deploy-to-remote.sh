...
echo "Deploying GoQuorum QBFT Network to remote server at $REMOTE_IP..."
...
cp -r ./generate-keys.sh ./generate-configs.sh ./docker-compose-remote.yaml ./SimpleStorage.sol ./FiatLoanMatcher.sol ./deploy-http.js ./deploy-fiatloan-http.js ./passwords.txt ./qbft-extradata.txt ./qbft-genesis.json $TEMP_DIR/goquorum-qbft-network/
...
cat > $TEMP_DIR/goquorum-qbft-network/setup-remote.sh << 'EOF'
#!/bin/bash
set -e

echo "Setting up GoQuorum QBFT Network on remote server..."
...
# Rename docker-compose-remote.yaml to docker-compose.yaml
cp docker-compose-remote.yaml docker-compose.yaml
...
chmod +x $TEMP_DIR/goquorum-qbft-network/setup-remote.sh

# Create the tar archive
echo "Creating tar archive..."
cd $TEMP_DIR
tar -czf goquorum-network.tar.gz goquorum-qbft-network/
...
sshpass -p "$REMOTE_PASSWORD" scp -o StrictHostKeyChecking=no $TEMP_DIR/goquorum-network.tar.gz $REMOTE_USER@$REMOTE_IP:/tmp/

# Execute commands on the remote server
echo "Setting up the network on the remote server..."
sshpass -p "$REMOTE_PASSWORD" ssh -o StrictHostKeyChecking=no $REMOTE_USER@$REMOTE_IP << EOF
  # Create the directory if it doesn't exist
  mkdir -p $REMOTE_DIR
  
  # Extract the archive
  tar -xzf /tmp/goquorum-network.tar.gz -C $REMOTE_DIR
  ...
  # Clean up
  rm /tmp/goquorum-network.tar.gz
EOF
